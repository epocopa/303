...
public class ProductoGUIImpl extends JPanel implements ProductoGUI, GUI {
	...
	public void initialize() {
		...
		JButton anadirBtn = createMenuButton(
				"resources/icons/productos/anadir-producto.png", new Color(91,
						155, 213));
		...
		JButton editarBtn = createMenuButton(
				"resources/icons/productos/editar-producto.png", new Color(255,
						192, 0));
		...
		JButton borrarBtn = createMenuButton(
				"resources/icons/productos/eliminar-producto.png", new Color(
						112, 173, 71));
		...
		JButton listarBtn = createMenuButton(
				"resources/icons/productos/mostrar-productos.png", new Color(
						234, 80, 54));
		listarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Context contexto = new Context(
						EventosProducto.LISTAR_PRODUCTOS, null);
		...
		JButton buscarBtn = createMenuButton(
				"resources/icons/productos/buscar-producto.png", new Color(47,
						85, 151));
		...
		enviarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					boolean calzado = Boolean.parseBoolean(esCalzadoField
							.getText());
					if (nombre.length() > 0 && !nombre.equals("") && precio > 0
							&& !precioField.equals("") && cantidad > 0
							&& !cantidadField.equals("")
							&& !esCalzadoField.equals("")) {
						if (calzado == true) {
							String respuesta = JOptionPane.showInputDialog(
									null, "Introduzca el numero de pie",
									"Finalizar operacion",
									JOptionPane.INFORMATION_MESSAGE);
							if (respuesta != null) {
								int numPie = Integer.parseInt(respuesta);
								TProductoCalzado pCalzado = new TProductoCalzado(
										0, nombre, cantidad, precio, numPie,
										true);
								Context contexto1 = new Context(
										EventosProducto.ANADIR_PRODUCTO,
										pCalzado);
								ControladorAplicacion.getInstance().accion(
										contexto1);
							}
						} else {
							String tejido = JOptionPane.showInputDialog(null,
									"Introduzca el tipo de tejido",
									"Finalizar operacion",
									JOptionPane.INFORMATION_MESSAGE);
							if (tejido != null) {
								TProductoTextil pTextil = new TProductoTextil(
										0, nombre, cantidad, precio, tejido,
										true);
								Context contexto1 = new Context(
										EventosProducto.ANADIR_PRODUCTO,
										pTextil);
		...
		buscarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (ID.length() > 0 && !ID.equals(" ")) {
					Context contexto = new Context(
							EventosProducto.MODIFICAR_BUSCAR_PRODUCTO, Integer
									.valueOf(ID));
					ControladorAplicacion.getInstance().accion(contexto);
				} else {
		...
		confirmBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if ((nombre.length() > 0 && !nombre.equals(""))
						&& (precio.length() > 0 && !precio.equals(""))
						&& (cantidad.length() > 0 && !cantidad.equals(""))
						&& (activo.length() > 0 && !activo.equals(""))
						&& (npie.length() > 0 && !npie.equals(""))
						&& (tejido.length() > 0 && !tejido.equals(""))) {
					if (npie.equalsIgnoreCase("N/A")) {
						TProductoTextil pTextil = new TProductoTextil(ID,
								nombre, Integer.valueOf(cantidad), Double
										.valueOf(precio), tejido, Boolean
										.valueOf(activo));
						Context contexto = new Context(
								EventosProducto.MODIFICAR_PRODUCTO, pTextil);
						ControladorAplicacion.getInstance().accion(contexto);
					} else {
						TProductoCalzado pCalzado = new TProductoCalzado(ID,
								nombre, Integer.valueOf(cantidad), Double
										.valueOf(precio),
								Integer.valueOf(npie), Boolean.valueOf(activo));
						Context contexto = new Context(
								EventosProducto.MODIFICAR_PRODUCTO, pCalzado);
						ControladorAplicacion.getInstance().accion(contexto);
					}
				} else {
					showOutputMsg(editarBuscarErrorArea,
	...
	public void borrarPanel() {
		...
		borrarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (ID.length() > 0 && !ID.equals(" ")) {
					Context contexto = new Context(
							EventosProducto.BAJA_PRODUCTO, Integer.valueOf(ID));
					ControladorAplicacion.getInstance().accion(contexto);
				} else {
	...
	public void mostrarPanel() {
		...
		String[] columns = { "ID Producto", "Nombre", "Precio", "Cantidad",
				"Numero de pie", "Tejido", "Activo" };
	...
	public void buscarPanel() {
		...
		buscarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (ID.length() > 0 && !ID.equals(" ")) {
					Context contexto = new Context(
							EventosProducto.MOSTRAR_PRODUCTO, Integer
									.valueOf(ID));
					ControladorAplicacion.getInstance().accion(contexto);
				} else {
	...
	public void actualizar(int evento, Object datos) {
		...
		case EventosProducto.MOSTRAR_PRODUCTO_OK:
			...
			mostrarPrecioText.setText(Double.valueOf(producto.getPrecio())
					.toString());
		...
		case EventosProducto.LISTAR_PRODUCTOS_OK:
			...
			List<TProducto> listaProductos = (List<TProducto>) datos;
			addPathSeparator();
			...
			for (TProducto p : listaProductos) {
				if (p.isCalzado() == true) {
					...
					mostrarModel.addRow(new Object[] { p.getId(),
							p.getNombre(), Double.valueOf(p.getPrecio()),
							p.getCantidad(), pCalzado.getNumero(), "N/A",
							p.isActivo() });
				} else {
					TProductoTextil pTextil = (TProductoTextil) p;
					mostrarModel.addRow(new Object[] { p.getId(),
							p.getNombre(), Double.valueOf(p.getPrecio()),
							p.getCantidad(), "N/A", pTextil.getTejido(),
							p.isActivo() });
				}
			}
		...
		case EventosProducto.MODIFICAR_BUSCAR_PRODUCTO_OK:
			...
			editarPrecioField.setText(Double.valueOf(producto.getPrecio())
					.toString());
			editarCantidadField.setText(Integer.valueOf(producto.getCantidad())
					.toString());
			editarActivoField.setText(Boolean.toString(producto.isActivo()));

			if (producto.isCalzado() == true) {
				TProductoCalzado pCalzado = (TProductoCalzado) datos;
				editarNumPieField.setText(Integer.valueOf(pCalzado.getNumero())
						.toString());
