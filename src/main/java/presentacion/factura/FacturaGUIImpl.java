package presentacion.facturaGUI;
...
public class FacturaGUIImpl extends JPanel implements FacturaGUI {
	...
	public void initialize() {
			...
			public void actionPerformed(ActionEvent e){				
				...
				if (result == JOptionPane.OK_OPTION) {
					...
					if ((IDClienteText.length() > 0 && !IDClienteText.equals(" "))) {
						TFactura factura = new TFactura(0, 0.0, 0.0,  null, Integer.valueOf(IDClienteText));
		...
		c.gridx++;
		JButton facturasBtn = createMenuButton("resources/icons/clientes/facturas-cliente.png", new Color(47, 133, 28));	// CAMBIAR IMAGEN
	...
	public void abrirPanel() {
			...
			public void actionPerformed(ActionEvent e){
				...
				if (result == JOptionPane.OK_OPTION) {
					...
					if ((descuentoAplicado.length() > 0 && !descuentoAplicado.equals(" "))) {
						TFactura factura = new TFactura(Id, Double.valueOf(descuentoAplicado), 0.0,  null, 0);
		...
		confirmar.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				final JComponent[] input = new JComponent[] {
				        new JLabel("�Desea enviar y cerrar la factura?"),
				};
				...
				if (result == JOptionPane.OK_OPTION) {
					TFactura factura = new TFactura(Id, 0.0, Double.valueOf(totalPrecio.getText()), null, 0);
		...
		switch (evento){
			...
			case EventosFactura.FACTURAS_CLIENTE_OK:
				...
				for (TFactura f : listaFacturas) {
					facturasModel.addRow(new Object[]{f.getId().toString(), f.getIdCliente().toString(), Double.valueOf(f.getDescuento()).toString(), Double.valueOf(f.getPrecio()).toString(), f.getFecha().toString()});
				}
