...
public class TurnoGUIImpl extends JPanel implements TurnoGUI, GUI {
	...
	public void initialize() {
		...
		JButton anadirBtn = createMenuButton(
				"resources/icons/turno/anadir-turno.png", new Color(91, 155,
						213));
		...
		JButton editarBtn = createMenuButton(
				"resources/icons/turno/editar-turno.png",
				new Color(255, 192, 0));
		...
		JButton borrarBtn = createMenuButton(
				"resources/icons/turno/eliminar-turno.png", new Color(112, 173,
						71));
		...
		JButton listarBtn = createMenuButton(
				"resources/icons/turno/mostrar-turnos.png", new Color(234, 80,
						54));
		...
		JButton buscarBtn = createMenuButton(
				"resources/icons/turno/buscar-turno.png",
				new Color(47, 85, 151));
	...
	public void anadirPanel() {
		...
		c.gridy++;
		JLabel horaInicioLabel = new JLabel("Hora inicio (hh:mm:ss): ");
		horaInicioLabel.setFont(new Font("Arial", Font.PLAIN, 18));
		formPanel.add(horaInicioLabel, c);

		c.gridy++;
		JLabel horaFinLabel = new JLabel("Hora fin (hh:mm:ss): ");
		...
		enviarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					LocalTime horaInicio = LocalTime.parse(horaInicioField
							.getText());
					LocalTime horaFin = LocalTime.parse(horaFinField.getText());
					if (nombre.length() > 0 && !nombre.equals("")
							&& horaInicioField.getText().length() > 0
							&& !horaInicioField.getText().equals("")
							&& horaFinField.getText().length() > 0
							&& !horaFinField.getText().equals("")) {
						turno.setNombre(nombre);
						turno.setHoraInicio(horaInicio);
						turno.setHoraFin(horaFin);
						turno.setActivo(true);
						Context contexto = new Context(
								EventosTurno.ANADIR_TURNO, turno);
						ControladorAplicacion.getInstance().accion(contexto);
					} else {
	...
	public void editarPanel() {
		...
		buscarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (ID.length() > 0 && !ID.equals(" ")) {
					Context contexto = new Context(
							EventosTurno.MODIFICAR_BUSCAR_TURNO, Integer
									.valueOf(ID));
					ControladorAplicacion.getInstance().accion(contexto);
				} else {
		...
		confirmBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				LocalTime horaInicio = LocalTime.parse(editarHoraInicioField
						.getText());
				LocalTime horaFin = LocalTime.parse(editarHoraFinField
						.getText());
				Boolean activo = Boolean.valueOf(editarActivoField.getText());

				if (nombre.length() > 0 && !nombre.equals("")
						&& horaInicio.toString().length() > 0
						&& !horaInicio.toString().equals("")
						&& horaFin.toString().length() > 0
						&& !horaFin.toString().equals("")
						&& activo.toString().length() > 0
						&& !activo.toString().equals("")) {
					TTurno turno = new TTurno(ID, nombre, horaInicio, horaFin,
							activo);
					Context contexto = new Context(
							EventosTurno.MODIFICAR_TURNO, turno);
					ControladorAplicacion.getInstance().accion(contexto);
				} else {
					showOutputMsg(editarBuscarErrorArea,
							editarBuscarErrorLabel,
	...
	public void borrarPanel() {
		...
		borrarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String ID = IDField.getText();
				if (ID.length() > 0 && !ID.equals(" ")) {
					Context contexto = new Context(EventosTurno.BAJA_TURNO,
							Integer.valueOf(ID));
					ControladorAplicacion.getInstance().accion(contexto);
				} else {
	...
	public void mostrarPanel() {
		...
		String[] columns = { "ID Turno", "Nombre", "Hora Inicio", "Hora Fin",
				"Activo" };
	...
	public void buscarPanel() {
		...
		buscarBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String ID = IDField.getText();
				if (ID.length() > 0 && !ID.equals(" ")) {
					Context contexto = new Context(EventosTurno.MOSTRAR_TURNO,
							Integer.valueOf(ID));
					ControladorAplicacion.getInstance().accion(contexto);
				} else {
	...
	public void actualizar(int evento, Object datos) {
		...
		case EventosTurno.LISTAR_TURNO_OK:
			...
			List<TTurno> listaTurnos = (List<TTurno>) datos;
			addPathSeparator();
			createPathButton("LISTAR TURNOS");
			mostrarPanel();

			for (TTurno t : listaTurnos) {
				mostrarModel.addRow(new Object[] { t.getId(), t.getNombre(),
						t.getHoraInicio().toString(),
						t.getHoraFin().toString(), t.isActivo() });
			}
