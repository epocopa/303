...
public class TurnoGUIImpl extends JPanel implements TurnoGUI, GUI{
	...
	public void initialize() {
		...
		c.gridx++;
		JButton anadirEmpleadoBtn = createMenuButton("resources/icons/turno/anadir-empleado.png", new Color(146, 208, 80));
		...
		c.gridx++;
		JButton bajaEmpleadoBtn = createMenuButton("resources/icons/turno/baja-empleado.png", new Color(0, 176, 240));
		bajaEmpleadoBtn.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				addPathSeparator();
				createPathButton("BAJA EMPLEADO");
				bajaEmpleadoPanel();
			}
		});
		_homePanel.add(bajaEmpleadoBtn, c);
	...
	public void anadirEmpleadoPanel() {
		...
		JButton borrarBtn = new JButton("ANADIR");
		borrarBtn.setFocusPainted(false);
		borrarBtn.setFont(new Font("Arial", Font.PLAIN, 18));
		borrarBtn.setBackground(new Color(230,230,230));
		borrarBtn.setMaximumSize(new Dimension(125, 30));
		...
		borrarBtn.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				if (ID.length() > 0 && !ID.equals(" ") && IDEmpleado.length() > 0 && !IDEmpleado.equals(" ")) {
					TEmpleado empleado = new TEmpleado();
					empleado.setId(Integer.parseInt(IDEmpleado));
					empleado.setIdTurno(Integer.parseInt(ID));
					Context contexto = new Context(EventosTurno.ANADIR_EMPLEADO_A_TURNO, empleado);
					ControladorAplicacion.getInstance().accion(contexto);
				} else {
	...
	public void bajaEmpleadoPanel() {
		...
		borrarEmpleadoOutputArea.add(borrarOutputLabel);
		...
		borrarBtn.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				if (ID.length() > 0 && !ID.equals(" ") && IDEmpleado.length() > 0 && !IDEmpleado.equals(" ")) {
					...
					empleado.setId(Integer.parseInt(IDEmpleado));
					empleado.setIdTurno(Integer.parseInt(ID));
					Context contexto = new Context(EventosTurno.BAJA_EMPLEADO_A_TURNO, empleado);
					ControladorAplicacion.getInstance().accion(contexto);
				} else {
					showOutputMsg(borrarEmpleadoOutputArea, borrarOutputLabel, "ERROR: El ID introducido no es valido.", false);
				}
			}
		});
	...
	public void actualizar(int evento, Object datos) {
		...
		switch (evento){
			...
			case EventosTurno.ANADIR_EMPLEADO_A_TURNO_OK:
				...
				System.out.println("Anadir Empleado A Turno OK");
				break;
